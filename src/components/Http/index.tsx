...
const Http: FC<{ mode?: "normal" | "compare" }> = ({ mode = "normal" }) => {
  ...
  const { data: response, run: request } = useRequest(
    axios,
    {
      manual: true,
      onSuccess: (res) => {
  ...
  const handleRequest = () => {
    if (!url) {
      return message.warn("Please input url");
    }
  ...
  const getColumns = (target: "params" | "requestHeader"): ColumnsType<
    ParamsType
  > => {
